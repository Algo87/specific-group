...
//Параметры
define('PASS', 'exstreme'); //пароль (желательно поменять на свой). Для запуска скрипта, в браузер введите адрес http://example.com/name.php?pass=pass, где example.com - адрес вашего сайта, pass - ваш пароль
define('EXT', 'gif,jpg,jpeg,png,zip,rar,pdf,css,flv,mp3,mp4,swf,jpa,gz,tar,bz,xls,doc,docx,xlsx'); //расширения файлов, в которых не производить поиск
define('CNT_STR', 100); //количество выводимых символов в результате поиска (до искомой фразы и после)
define('FILE_SIZE', 10000000); //максимальный размер файла
...
function scan_dir($dirname) { 
	...
	global $text, $replace,$delempty, $ext, $cnt, $html, $regex, $regis;
	...
	while (($file = readdir($dir)) !== false) {
		...
		if ($file != '.' && $file != '..') {
			...
			if (is_file($file_name)) {
				...
				{
					echo "Пропускаем файл $file_name<br>";
					continue;
				}
				...
				if ($str != '') {
					...
					if ($replace) {
						...
						if ($delempty) {
						if(filesize(DIR_NAME.DIRECTORY_SEPARATOR.$file_name)==0){
						$file = DIR_NAME.DIRECTORY_SEPARATOR.$file_name;
	...
	if (is_file($file) == true) {
		...
		if (@unlink($file) == false) {
			...
			echo 'Файл '.mb_convert_encoding($file,'utf-8','windows-1251').' удален<br>';	
		}
	} else {
		echo 'Ошибка: не удалось удалить файл';
	}
...
if (isset($_POST['submit'])) {
	...
	$delempty = isset($_POST['delempty']) ? 1 : 0;
...
<?php 
    ...
    <form method="post" action="">
        <table border="0">
			...
			<tr>
                <td align="right">
                    <label>Удалить пустые файлы</label>
                </td>
                <td>
                    <input type="checkbox"<?php echo isset($delempty) && $delempty == 1 ? ' checked' : ''; ?> name="delempty" value="1" />
                </td>
            </tr>
            <tr>
                <td align="right">
...
<div class="footer">
	<div class="copy">
    	&copy; <?php echo date('Y'); ?> <a href="http://protectyoursite.ru/">protectyoursite.ru</a>
	</div>
</div>
</body>
</html>